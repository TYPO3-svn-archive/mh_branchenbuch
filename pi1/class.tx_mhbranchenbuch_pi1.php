...
class tx_mhbranchenbuch_pi1 extends tslib_pibase {
  ...
  var $freeCap = FALSE;
    ...
    $sortBy   = $this->listSort ? $this->listSort : 'tx_mhbranchenbuch_firmen.firma ASC';
    ...
    if($catId) {
      ...
      foreach($catId AS $value) {
        ...
        $query    .= 'FIND_IN_SET(' . intval($value) . ',tx_mhbranchenbuch_firmen.kategorie)';
    ...
    if($listFederal) {
      ...
      foreach($listFederal AS $federal) {
        ...
        $query    .= 'FIND_IN_SET(' . intval($federal) . ',tx_mhbranchenbuch_firmen.bundesland)';
    ...
    if($listAdminis) {
      ...
      foreach($listAdminis AS $adminis) {
        ...
        $query    .= 'FIND_IN_SET(' . intval($adminis) . ',tx_mhbranchenbuch_firmen.landkreis)';
    ...
    if($listCity) {
      ...
      foreach($listCity AS $city) {
        ...
        $query    .= 'FIND_IN_SET(' . intval($city) . ',tx_mhbranchenbuch_firmen.ort)';
    ...
    $res = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
      SELECT
        ...
        o.name AS o_city
      FROM
        " . $this->dbTable1 . " tx_mhbranchenbuch_firmen
        JOIN " . $this->dbTable2 . " k ON k.uid = tx_mhbranchenbuch_firmen.kategorie
        JOIN " . $this->dbTable5 . " o ON o.uid = tx_mhbranchenbuch_firmen.ort
      WHERE
        tx_mhbranchenbuch_firmen.pid IN (" . intval($pid) . ") 
        " . $query . "
        " . $this->cObj->enableFields($this->dbTable1) . "
      ORDER BY
        " . $sortBy . "
      LIMIT " . $limit . "
    ");
  ...
  */
  function displaySearch($pid) {
    ...
    if($query) 
    {
      $orderBy = isset($this->searchSort) ? " FIND_IN_SET(tx_mhbranchenbuch_firmen.typ,'" . $this->searchSort . "'), tx_mhbranchenbuch_firmen.firma ASC " : ' tx_mhbranchenbuch_firmen.firma ASC ';
    ...
    foreach($keyword AS $searchString) {
      ...
      foreach($sTable AS $search_table) {
        if($c<($sTableCount*$cPVar)) {
          $query .= "tx_mhbranchenbuch_firmen." . $search_table . " LIKE " . $GLOBALS['TYPO3_DB']->fullQuoteStr('%'.$searchString.'%',$this->dbTable1) . " OR ";
          $c++; 
        }
      }
  ...
  function getSearchResultsKeyword2($keyword) {
    ...
    $lookKeyword  = ''; #init
    $lookKeyword  .= count($inCity) > 0 ? " AND FIND_IN_SET(tx_mhbranchenbuch_firmen.ort,'" . implode(',',$inCity) . "') "  : FALSE;
    $lookKeyword  .= count($inAdminDistrict) > 0 ? " AND FIND_IN_SET(tx_mhbranchenbuch_firmen.landkreis,'" . implode(',',$inAdminDistrict) . "') "  : FALSE;
    
    $showXS = $this->search_showXS == 1 ? FALSE : ' AND tx_mhbranchenbuch_firmen.typ != 7';
  ...
  */
  function displayLatest($pid,$catId) {    
    ...
    $res = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
      ...
      FROM
        " . $this->dbTable1 . " tx_mhbranchenbuch_firmen
        LEFT JOIN " . $this->dbTable5 . " o ON o.uid = tx_mhbranchenbuch_firmen.ort
        LEFT JOIN " . $this->dbTable2 . " k ON k.uid = tx_mhbranchenbuch_firmen.kategorie
      WHERE
        tx_mhbranchenbuch_firmen.pid IN (" . intval($pid) . ")
        " . $query . "
        " . $this->cObj->enableFields($this->dbTable1) . "
      ORDER BY
        tx_mhbranchenbuch_firmen.crdate DESC
  ...
  function displayTagCloud($pid) {
    ...
    $res    = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
      ...
      FROM 
        " . $this->dbTable1 . " tx_mhbranchenbuch_firmen
      LEFT JOIN 
        " . $this->dbTable2 . " k ON FIND_IN_SET(k.uid, tx_mhbranchenbuch_firmen.kategorie)
      WHERE 
        f.pid IN (" . intval($pid) . ")
        " . $search . "
        " . $this->cObj->enableFields($this->dbTable1) . "
    ...
    if(in_array($postVarLetter,$alphabetic) OR $postVarLetter == 'all') {
      ...
      $res2 = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
        ...
        WHERE
          ...
          " . $this->cObj->enableFields($this->dbTable1) . "
        ORDER BY
          tx_mhbranchenbuch_firmen.firma
      ");
  ...
  function getItem($res, $full = FALSE, $detail = FALSE, $pageBrowser = FALSE) {
    ...
    if($GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
      while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
        ...
        $markerArray['###STARTTIME###'] = date($this->datetime,$row['starttime']);
        $markerArray['###ENDTIME###']   = date($this->datetime,$row['endtime']);
        ...
        if($row['catImage']) {
      	...
      	// Image Settings
      	if ($row['bild'] == FALSE) {
          $picture = $this->conf['noImage'];
        } else {
          $pic_temp = @explode(',', $row['bild']);
        	if(is_array($pic_temp)) { 
            $picture = 'uploads/tx_mhbranchenbuch/' . $pic_temp[0];
          } else {
            $picture = 'uploads/tx_mhbranchenbuch/' . $row['bild'];
          }
        }
    ...
    if($this->maxEntriesPerUser > 0 && $this->getEntriesPerUser($GLOBALS['TSFE']->fe_user->user['uid']) >= $this->maxEntriesPerUser) {
      ...
      if($postFormId > 0) {
        ...
        if(!$error) {
          ...
          $images = array(); #init
          $image_upload = $_FILES['tx_mhbranchenbuch_images'];
          
          for($image_count=0;$image_count<count($image_upload['name']);$image_count++) {
            if($image_upload['name'][$image_count] != '') {
              $cleanFile  = $this->fileFunc->cleanFileName($image_upload['name'][$image_count]);
              $uniqueName = $this->fileFunc->getUniqueName($cleanFile, "uploads/tx_mhbranchenbuch/");
              // Check imagesize
              $fileInfo = $this->fileFunc->getTotalFileInfo($image_upload['tmp_name'][$image_count]);
              if(($fileInfo['size']/1024) <= $this->feForm_maxsize) {
                move_uploaded_file($image_upload['tmp_name'][$image_count],$uniqueName);
                $temp_unique  = explode('/',$uniqueName);   
                $images[]     = $temp_unique[2];
              }
            }
          }
          ...
          // Get latitude & longitude for GMaps
          if($this->map_api != '' && version_compare(PHP_VERSION, '5.0.0', '>')) {
            $GmapStreet = $postVar['anschrift'];
            $GmapCity   = $postVar['zip'] . ' ' . $postVar['city'];
            $GMapAdress =  urlencode("$GmapStreet $GmapCity");
  
            $request_url = 'http://maps.google.com/maps/geo?q=' . $GMapAdress . '&output=xml&key=' . $this->map_api;
            $page = file_get_contents($request_url);
            $xml = new SimpleXMLElement($page);
            list($longitude, $latitude, $altitude) = explode(",",$xml->Response->Placemark->Point->coordinates);
            
            $insertArray['map_lat'] = $latitude;
            $insertArray['map_lng'] = $longitude;
          }
        ...
        if($oid) {
          if($oid == 'create') {
            ...
            if(array_key_exists($type,$type_valid)) {
              ...
              // Get upload fields ...
              $upload_fields  = $this->feForm_uploadFields;
              $upload_field   = ''; #init
              for($count_temp = 1; $count_temp <= $upload_fields; $count_temp++) {
                $upload_field .= '<label for="upload' . $count_temp . '">'. $this->pi_getLL('feform_upload_choose') . '</label><input type="file" id="upload' . $count_temp . '" size="30" name="tx_mhbranchenbuch_images[]" accept="*.jpg,*.gif,*.jpeg" />';
              }
  ...
  function getLID($bid,$single_pid = FALSE) {
    ...
    $sql = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
      ...
      WHERE 
        ...
        " . $this->cObj->enableFields($this->dbTable4) . "
      ORDER BY name
    ");
  ...
  */
  function getOID($lid,$bid) {
    ...
    $sql = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
      ...
      WHERE 
        landkreis = " . intval($lid) . "
        " . $this->cObj->enableFields($this->dbTable5) . "
      ORDER BY 
        name
    ");
  ...
  */
  function displayOverview($pid,$bid,$lid,$oid,$kid = FALSE,$catId = FALSE) {
    ...
    if($kid) {
      ...
      $orderBy = isset($this->overviewSort) ? " FIND_IN_SET(tx_mhbranchenbuch_firmen.typ,'" . $this->overviewSort . "'), tx_mhbranchenbuch_firmen.firma ASC " : ' tx_mhbranchenbuch_firmen.firma ASC ';
      ...
      $res = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
        ...
        WHERE
          tx_mhbranchenbuch_firmen.bundesland = " . intval($bid) . "
          AND
          tx_mhbranchenbuch_firmen.landkreis = " . intval($lid) . "
          AND
          tx_mhbranchenbuch_firmen.ort = " . intval($oid) . "
          AND
          FIND_IN_SET(" . intval($kid) . ",tx_mhbranchenbuch_firmen.kategorie)
          AND
          tx_mhbranchenbuch_firmen.pid IN (" . intval($pid) . ")
          " . $this->cObj->enableFields($this->dbTable1) . "
        ORDER BY
  ...
  */
  function initMap($lat, $lng, $lid = FALSE, $bid = FALSE, $oid = FALSE, $uid = FALSE) {
    ...
    $WHERE_CLAUSE = ''; #init
    if($bid) { $WHERE_CLAUSE .= 'tx_mhbranchenbuch_firmen.`bundesland` = ' . intval($bid); $zoom = $this->map_zoom1; }
    if($lid) { $WHERE_CLAUSE .= ' tx_mhbranchenbuch_firmen.`landkreis` = ' . intval($lid); $zoom = $this->map_zoom2; }
    if($oid) { $WHERE_CLAUSE .= ' tx_mhbranchenbuch_firmen.`ort` = ' . intval($oid); $zoom = $this->map_zoom3; }
    if($uid) { $WHERE_CLAUSE .= ' tx_mhbranchenbuch_firmen.`uid` = ' . intval($uid); $zoom = $this->map_zoom4; }
    ...
    if($api) {
      ...
      $sql = $GLOBALS['TYPO3_DB']->sql(TYPO3_db,"
        ...
        WHERE 
          " . $WHERE_CLAUSE . "
        AND
          tx_mhbranchenbuch_firmen.`map_lat` != ''
        AND
          tx_mhbranchenbuch_firmen.`map_lng` != ''
        " . $this->cObj->enableFields($this->dbTable1)
      );
      ...
      if($GLOBALS['TYPO3_DB']->sql_num_rows($sql)) {
        while($row = mysql_fetch_array($sql)) {
          $marker_content = $this->cObj->getTypoLink(htmlspecialchars($row['firma'],ENT_QUOTES),$row['link'],'', $this->conf['linkTarget']);
